...
| Artifact attestation verification | Release manager | Workflow: `.github/workflows/release.yml` publish job generates release artifact attestations using `actions/attest@v4`. Command: `gh attestation verify <artifact> -R semantic-reasoning/PyreWire` for every release wheel (`dist/pyrewire-*.whl`) and the sdist (`dist/pyrewire-*.tar.gz`). | Successful `release.yml` run URL, release tag or frozen RC commit SHA, artifact filenames, SHA256 hashes for each wheel/sdist, and successful `gh attestation verify` output for each verified artifact. | Link a GitHub issue, PR, or follow-up task before retrying or tagging. |
...
Attestation verification proves provenance for each PyreWire release artifact digest, including the source repository/ref and release workflow run that produced it.
This does not independently attest upstream wirelog builds. For bundled wirelog traceability, rely on the pinned wirelog v0.44.0 SHA `5bebc8d40bbb850179fbb091807964762df5a814` in release configuration and docs, plus the wheel dynamic-link and clean-install gates above.
