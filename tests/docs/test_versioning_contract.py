...
WIRELOG_SHA_CURRENT_DEVELOPMENT = PINNED_WIRELOG_SHA
...
def test_versioning_documents_104_wirelog_pin_and_runtime_floor():
    minimum, validated_ref, notes = _versioning_row("1.0.4")
...
def test_versioning_documents_106_wirelog_pin_and_runtime_floor():
    minimum, validated_ref, notes = _versioning_row("1.0.6")

    assert minimum == "`0.52.0`"
    assert validated_ref == f"`{WIRELOG_SHA_106}`"
    assert "v0.60.0" in notes
    assert "runtime minimum remains `0.52.0`" in notes
    assert "peeled tag SHA" in notes


def test_versioning_documents_current_development_wirelog_pin_and_runtime_floor():
    minimum, validated_ref, notes = _versioning_row("current-development")

    assert minimum == "`0.52.0`"
    assert validated_ref == f"`{WIRELOG_SHA_CURRENT_DEVELOPMENT}`"
    assert "v0.62.0" in notes
    assert "runtime minimum remains `0.52.0`" in notes
    assert "peeled tag SHA" in notes
    assert "wirelog 0.61.0 changed arithmetic" in notes
    assert "which can change results" in notes
